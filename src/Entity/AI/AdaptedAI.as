    ...
    public class AdaptedAI extends BasicAI {
        ...
        public function updateAdapted():void {
            ...
            for each (node in nodeArray) { // 计算目标天体
                ...
                shipCount = (node.predictedGroupStrength(team) - node.predictedOppStrength(team)) / shipStrength;
            ...
            if (targets.length > 0) { // 目标天体存在时
                ...
                for each (node in nodeArray) { // 计算出兵天体
                    ...
                    node.aiStrength = -node.groupStrength(team) / shipStrength; // 将该天体己方强度记为己方船数的相反数
                ...
                while (true) {
                    targetNode = GeneralFunctions.getRandomMinByProperty(targets, "aiValue") as Node;
                    senderNode = GeneralFunctions.getRandomMinByProperty(senders, "aiStrength") as Node;
            ...
            for each (node in nodeArray) { // 计算目标天体
                nodeGroup = Globals.teamGroups[node.nodeData.team];
                if (nodeGroup == group && ((node.nodeData.type != NodeType.DIFFUSION || node.predictedTeamShipCount(team) > 10) || Globals.teamPops[team] >= Globals.teamCaps[team]) || node.nodeData.isAIinvisible)
                ...
                dy = node.nodeData.y - centerY;
                distance = Math.sqrt(dx * dx + dy * dy) + rng.nextNumber() * 32;
                shipCount = (node.predictedOppStrength(team) - node.predictedGroupStrength(team)) / shipStrength;
                node.aiValue = distance + shipCount;
                //trace(node.tag);
            ...
            if (targets.length > 0) {
                ...
                for each (node in nodeArray) { // 计算出兵天体
                    ...
                    if (node.predictedOppShipCount(team) > 0 && node.predictedGroupStrength(team) > node.predictedOppStrength(team))
                        continue; // 条件：没有敌方或预测己方强度低于敌方
                    node.aiStrength = -node.groupShipCount(team) / shipStrength;
                    if (EntityContainer.inAttackNodeCheck(node, team, NodeType.PULSECANNON, true)) {
                        if (node.aiValue > 0)
                            node.aiValue /= 10;
                        else
                ...
                while (true) {
                    ...
                    if (targetNode.nodeData.type == NodeType.DIFFUSION && Globals.teamGroups[targetNode.nodeData.team] == group)
                        ships = 10 - targetNode.predictedTeamShipCount(team); // 给己方扩散补兵
            ...
            for each (node in nodeArray) { // 计算出兵天体
                ...
                node.aiStrength = (-node.teamStrength(team) - node.oppStrength(team)) / shipStrength; // 计算己方和最强方的飞船总数
            ...
            if (senders.length > 0) {
                ...
                while (true) {
                    ...
                    if (senderNode.nodeLinks[team].indexOf(targetNode) == -1)
                        continue; // 条件：出兵天体与目标天体之间不被拦截
                    if (!senderNode.nodeData.isWarp && EntityContainer.isInBlackhole(senderNode, targetNode, team))
                        continue; // 路径上无有威胁的黑洞

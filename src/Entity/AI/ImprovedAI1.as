package Entity.AI {
    ...
    public class ImprovedAI1 extends BasicAI {
        public function ImprovedAI1(rng:Rng, actionDelay:Number, startDelay:Number) {
                ...
                while (true) {
                    ...
                    for each (node in targets) {
                        ...
                        if (node.nodeData.type == NodeType.CLONETURRET)
                            node.aiValue = -25;
                        if (senderNode.nodeData.type == NodeType.CLONETURRET && node.nodeData.type == NodeType.DIFFUSION)
                            node.aiValue = -50;
                        if (EntityContainer.inAttackNodeCheck(node, team, NodeType.PULSECANNON, true)) {
                            if (node.aiValue > 0)
                                node.aiValue *= 10;

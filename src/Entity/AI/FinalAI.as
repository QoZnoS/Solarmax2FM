package Entity.AI {
    ...
    public class FinalAI extends BasicAI {
        ...
        public function updateFinal():void {
            ...
            if (nodeArray[0].predictedOppShipCount(team) > 0)
                ...
                for each (node in nodeArray) {
                    ...
                    if (nodeGroup == group || node.nodeData.isAIinvisible)
                        continue; // 排除己方天体和障碍
                    if (node.nodeData.team == 0 && node.predictedOppShipCount(team) == 0 && node.predictedGroupShipCount(team) >= node.nodeData.size * 200)
                        continue; // 排除仅被己方以二倍标准兵力占据的中立天体
                    if (node.predictedOppShipCount(team) > 0 && node.predictedGroupShipCount(team) * 0.5 > node.predictedOppShipCount(team))
            ...
            if (targets.length > 0) {
                ...
                for each (node in nodeArray) { // 计算出兵天体
                    ...
                    node.aiStrength = -node.teamShipCount(team);
                    senders.push(node);
                }
                ...
                for each (targetNode in targets) {
                    for each (senderNode in senders) {
                        ...
                        if (senderNode.predictedOppShipCount(team) > senderNode.predictedGroupShipCount(team))
                            ships = senderNode.teamShipCount(team); // 预测出兵天体敌方兵力高于己方兵力时派出全部兵力
                        ...
                        if (senderNode.nodeData.type == NodeType.DILATOR)
                            NodeStaticLogic.sendAIShips(senderNode, team, targetNode, senderNode.teamShipCount(team) - 150); // 星核特殊出兵机制
                        else

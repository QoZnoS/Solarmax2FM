    ...
    public class SimpleAI extends BasicAI {
        ...
        public function updateSimple():void {
            ...
            for each (node in nodeArray) { // 计算目标天体
                ...
                if (node.predictedGroupShipCount(team) > node.predictedOppShipCount(team) * 2)
                ...
                strength = node.predictedGroupShipCount(team) - node.predictedOppShipCount(team); // 己方势力强度减去非己方势力强度
            ...
            if (targets.length > 0) { // 目标存在时，出兵防守
                ...
                for each (node in nodeArray) { // 统计出兵天体
                    ...
                    if (node.conflict && node.predictedGroupShipCount(team) > node.predictedOppShipCount(team))
                        continue; // 条件：没有战争或预测己方强度低于敌方
                    node.aiStrength = -node.teamShipCount(team); // 将该天体己方强度记为飞船数的相反数
                    senders.push(node);
                }
                ...
                for each (targetNode in targets) { // 防守判定
                    for each (senderNode in senders) {
                        ...
                        if (senderNode.teamShipCount(team) + targetNode.predictedGroupShipCount(team) <= targetNode.predictedOppShipCount(team))
                            continue; // 出兵条件：出兵天体的强度和目标天体的预测强度之和高于目标天体的预测敌方强度
                        // 飞船数：目标天体上预测敌方强度的二倍减去预测己方强度
                        ships = targetNode.predictedOppShipCount(team) * 2 - targetNode.predictedGroupShipCount(team);
            ...
            for each (node in nodeArray) { // 计算目标天体
                ...
                if (nodeGroup == group || node.nodeData.isAIinvisible)
                    continue;
                if (node.nodeData.team == 0 && node.predictedOppShipCount(team) == 0 && node.predictedGroupShipCount(team) > node.nodeData.size * 100)
                    continue; // 目标条件：不为中立或预测有非己方飞船或己方势力飞船不足100倍size（基本兵力上限）
                dx = node.nodeData.x - centerX;
                dy = node.nodeData.y - centerY;
                distance = Math.sqrt(dx * dx + dy * dy) + rng.nextNumber() * 32; // 计算距离，带32px随机数误差
                strength = node.predictedOppShipCount(team) - node.predictedGroupShipCount(team); // 计算敌方强度：预测敌方强度减去预测己方强度
            ...
            if (targets.length > 0) { // 目标存在时，出兵进攻
                ...
                for each (targetNode in targets) { // 进攻判定
                    for each (senderNode in senders) {
                        ...
                        ships = targetNode.predictedOppShipCount(team) * 2 - targetNode.predictedGroupShipCount(team) * 0.5;
                        if (targetNode.predictedOppShipCount(team) * 2 - targetNode.predictedGroupShipCount(team) * 0.5 < targetNode.nodeData.size * 200)
                            ships = targetNode.nodeData.size * 200; // 若出兵兵力不足二倍目标天体标准兵力，则增加至二倍目标天体标准兵力
                        if (senderNode.predictedOppShipCount(team) > senderNode.predictedGroupShipCount(team))
                            ships = senderNode.teamShipCount(team); // 若预测出兵天体所受敌方威胁高于其强度，则派出全部兵力

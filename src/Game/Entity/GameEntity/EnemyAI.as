    ...
    public class EnemyAI extends GameEntity {
        ...
        public function attackV1(_dt:Number):void {
            ...
            for each (var _senderNode:Node in senders) { // 出兵
                ...
                targets.sortOn("aiValue", 16);
                for each (_targetNode in targets) { // 再派兵
        ...
        public function hard_getTowerAttack(_Node1:Node, _Node2:Node):Number { // 高精度估损
            ...
            for each (_Node in game.nodes.active) {
                ...
                if (_Node.attackStrategy.attackRange != 0) {
                    ...
                    lineIntersectCircle(_start, _end, _current, _Node.attackStrategy.attackRange);
                    if (resultIntersects) {
                        if(!resultEnter) 
                            resultEnter = _start;
                        if(!resultExit) 
        ...
        private function lineIntersectCircle(_pointA:Point, _pointB:Point, _circleCenter:Point, _circleRadius:Number = 1):void { // 判断线与圆的关系并返回交点

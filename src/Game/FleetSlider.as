...
package Game
{
   ...
   public class FleetSlider extends Sprite
   {
      ...
      public function FleetSlider(_type:int) // 添加贴图
      {
         ...
         if (type == 3)
         {
            label = new TextField(boxWidth * 2, boxHeight, "100%", _font, -1, 16755370);
            label.pivotX = boxWidth;
            label.pivotY = label.y = boxHeight * 0.5;
            label.x = boxWidth * 0.5;
            quad1 = new Quad(thickness, touchHeight, 16755370);
            quad1.x = boxWidth * 0.5 - thickness * 0.5;
            quad2 = new Quad(thickness, touchHeight, 16755370);
            quad2.pivotY = quad2.y = touchHeight;
            quad2.x = boxWidth * 0.5 - thickness * 0.5;
         }
         else
         {
            label = new TextField(boxWidth, boxHeight * 2, "100%", _font, -1, 16755370);
      ...
      public function on_touch(_touch:TouchEvent):void // 点击改变分兵条
      {
         var _TouchArray:Vector.<Touch> = _touch.getTouches(touchQuad);
         if (!_TouchArray)
            return;
         if (_TouchArray.length == 1) // 确保只有一个触点
         {
            ...
            switch (_Touch.phase)
            {
               ...
               case "ended":
                  type == 3 ?
                     total = 1 - (_Touch.getLocation(this).y - boxHeight * 0.5) / (touchHeight - boxHeight) :
      ...
      public function update():void
      {
         if (type == 3)
         {
            label.y = box.y = boxHeight * 0.5 + (1 - total) * (touchHeight - boxHeight);
            label.text = int(total * 100).toString() + "%";
            quad1.setVertexPosition(1, thickness, label.y + boxHeight * 0.5);
            quad1.setVertexPosition(0, 0, label.y + boxHeight * 0.5);
            quad2.setVertexPosition(3, thickness, label.y - boxHeight * 0.5);
            quad2.setVertexPosition(2, 0, label.y - boxHeight * 0.5);
         }
         else
         {

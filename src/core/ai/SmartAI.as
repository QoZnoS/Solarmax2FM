    ...
    public class SmartAI extends BasicAI {
        ...
        public function updateSmart():void {
            ...
            for each (node in nodeArray) { // 计算目标天体
                ...
                if (node.nodeData.type == NodeType.DILATOR && SituationAssessor.teamStrength(node, team) > 0) {
                    node.unloadShips();
                    return;
                }
                if (node.nodeData.isAIinvisible)
                    continue;
                if (nodeGroup != group && SituationAssessor.predictedGroupStrength(node, team) == 0)
                    continue; // 条件1：为己方天体或有己方飞船（包括飞行中的）
                if (SituationAssessor.predictedOppStrength(node, team) == 0)
                    continue; // 条件2：有敌方
                ...
                strength = SituationAssessor.predictedGroupStrength(node, team) - SituationAssessor.predictedOppStrength(node, team);
            ...
            if (targets.length > 0) { // 目标天体存在时
                ...
                for each (node in nodeArray) { // 计算出兵天体
                    ...
                    if (node.nodeData.isAIinvisible)
                        continue;
                    if (node.aiTimers[team] > 0 || SituationAssessor.teamStrength(node, team) == 0)
                    ...
                    if (SituationAssessor.predictedOppStrength(node, team) > 0 && SituationAssessor.predictedGroupStrength(node, team) > SituationAssessor.predictedOppStrength(node, team))
                        continue; // 条件：没有敌方或预测己方强度低于敌方
                    node.aiStrength = -SituationAssessor.groupStrength(node, team); // 将该天体己方强度记为飞船数的相反数
                    senders.push(node);
                }
                ...
                for each (targetNode in targets) {
                    for each (senderNode in senders) {
                        ...
                        ships = SituationAssessor.predictedOppStrength(targetNode, team) * 2 - SituationAssessor.predictedGroupStrength(targetNode, team);
                        ...
                        if (towerAttack > 0 && SituationAssessor.teamStrength(senderNode, team) < towerAttack * 0.5)
            ...
            for each (node in nodeArray) { // 计算目标天体
                ...
                if (nodeGroup == group || node.nodeData.isAIinvisible)
                    continue;
                if (SituationAssessor.predictedOppStrength(node, team) == 0 && SituationAssessor.predictedGroupStrength(node, team) > node.nodeData.size * 150)
                    continue; // 条件：排除己方强度足够且无敌方的天体
                dx = node.nodeData.x - centerX;
                dy = node.nodeData.y - centerY;
                distence = Math.sqrt(dx * dx + dy * dy) + rng.nextNumber() * 32;
                strength = SituationAssessor.predictedOppStrength(node, team) - SituationAssessor.predictedGroupStrength(node, team);
                node.aiValue = distence + strength;
                targets.push(node);
            }
            ...
            if (targets.length > 0) {
                ...
                for each (node in nodeArray) { // 计算出兵天体
                    ...
                    if (node.aiTimers[team] > 0 || SituationAssessor.teamStrength(node, team) == 0)
                        continue; // 基本条件：该天体己方ai倒计时为0且该天体己方强度不为0
                    if (SituationAssessor.predictedOppStrength(node, team) == 0 && node.capturing)
                ...
                for each (targetNode in targets) {
                    for each (senderNode in senders) {
                        ...
                        ships = SituationAssessor.predictedOppStrength(targetNode, team) * 2 - SituationAssessor.predictedGroupStrength(targetNode, team) * 0.5;
                        if (SituationAssessor.predictedOppStrength(senderNode, team) > SituationAssessor.predictedGroupStrength(senderNode, team))
                            ships = SituationAssessor.teamStrength(senderNode, team); // 预测敌方强度大于己方时，派出全部飞船
                        ...
                        if (Globals.level == 31)
                            if (towerAttack > 0 && SituationAssessor.teamStrength(senderNode, team) < towerAttack * 2)
            ...
            for each (node in nodeArray) { // 计算出兵天体
                ...
                if (node.nodeData.isAIinvisible)
                    continue;
                if (nodeGroup != group && SituationAssessor.predictedOppStrength(node, team) == 0 && SituationAssessor.groupStrength(node, team) > 0)
                    continue; // 条件：没在锁星
                if (SituationAssessor.predictedOppStrength(node, team) > 0 && SituationAssessor.predictedGroupStrength(node, team) > SituationAssessor.predictedOppStrength(node, team))
                    continue; // 条件：无敌方或打不过敌方
                node.aiStrength = -SituationAssessor.teamStrength(node, team) - SituationAssessor.oppStrength(node, team); // 计算己方和最强方的飞船总数
                node.aiValue = -node.oppNodeLinks[team].length; // 按路径数计算价值
            ...
            if (senders.length > 0) {
                ...
                for each (node in nodeArray) { // 计算目标天体
                    ...
                    if (node.nodeData.isWarp)
                        node.aiValue--; // 传送权重提高
                    if (node.nodeData.type == NodeType.DIFFUSION)
                        node.aiValue--; // 扩散权重提高
                ...
                for each (targetNode in targets) {
                    for each (senderNode in senders) {
                        ...
                        ships = SituationAssessor.teamStrength(senderNode, team); // 派出全部飞船
                        towerAttack = EntityContainer.getLengthInTowerRange(senderNode, targetNode, team) / 4.5;
                        ships += towerAttack; // 为飞船数加上估损
                        if (towerAttack > 0 && Globals.teamPops[team] < towerAttack)
                            continue; // 条件：总兵力不足估损时不派兵
                        if (Globals.level == 31)
                            if (towerAttack > 0 && SituationAssessor.teamStrength(senderNode, team) < towerAttack * 3)

        ...
        public function updateDark():void {
            ...
            for each (node in nodeArray) { // 计算目标天体
                ...
                if (nodeGroup == group || node.nodeData.isAIinvisible)
                    continue;
                if (SituationAssessor.predictedOppStrength(node, team) == 0 && SituationAssessor.predictedGroupStrength(node, team) > node.nodeData.size * 200)
                    continue; // 条件1：天体未被己方以二倍标准兵力占据
                if (SituationAssessor.predictedOppStrength(node, team) > 0 && SituationAssessor.predictedGroupStrength(node, team) * 0.5 > SituationAssessor.predictedOppStrength(node, team))
                ...
                for each (targetNode in targets) {
                    for each (senderNode in senders) {
                        ...
                        if (SituationAssessor.predictedOppStrength(senderNode, team) > SituationAssessor.predictedGroupStrength(senderNode, team))
                            ships = SituationAssessor.teamStrength(senderNode, team); // 预测出兵天体敌方兵力高于己方兵力时派出全部兵力
            ...
            for each (node in nodeArray) { // 计算出兵天体
                nodeGroup = Globals.teamGroups[node.nodeData.team];
                if (nodeGroup != group || node.conflict || SituationAssessor.teamStrength(node, team) == 0)
                    continue; // 条件：为己方天体且无战争
                node.aiValue = -SituationAssessor.teamStrength(node, team);
                senders.push(node);
            }
            ...
            if (senders.length > 0) {
                ...
                for each (senderNode in senders) {
                    for each (targetNode in targets) {
                        ...
                        ships = SituationAssessor.teamStrength(senderNode, team); // 派出该天体全部兵力
                        towerAttack = EntityContainer.getLengthInTowerRange(senderNode, targetNode, team) / 4.5;
                        ships += towerAttack; // 加上估损
                        if (towerAttack > 0 && Globals.teamPops[team] < towerAttack)
                            continue; // 总兵力不足估损时不派兵

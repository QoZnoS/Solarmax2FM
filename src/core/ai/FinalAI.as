package core.ai {
    ...
    public class FinalAI extends BasicAI {
        ...
        public function updateFinal():void {
            ...
            if (SituationAssessor.predictedOppStrength(nodeArray[0], team) > 0)
                ...
                for each (node in nodeArray) {
                    ...
                    if (nodeGroup == group || node.nodeData.isAIinvisible)
                        continue; // 排除己方天体和障碍
                    if (node.nodeData.team == 0 && SituationAssessor.predictedOppStrength(node, team) == 0 && SituationAssessor.predictedGroupStrength(node, team) >= node.nodeData.size * 200)
                ...
                for each (targetNode in targets) {
                    for each (senderNode in senders) {
                        ...
                        if (senderNode.nodeData.type == NodeType.DILATOR)
                            NodeStaticLogic.sendAIShips(senderNode, team, targetNode, SituationAssessor.teamStrength(senderNode, team) - 150); // 星核特殊出兵机制
                        else

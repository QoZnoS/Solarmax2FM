        ...
        public function updateRadical():void {
            ...
            if (senders.length > 0) {
                ...
                while (targets.length > 0 && senders.length > 0) {
                    ...
                    if (targetNode && senderNode) {
                        ...
                        if (towerAttack > 0 && SituationAssessor.teamStrength(senderNode, team, false) < towerAttack * 0.9) {
            ...
            for each (node in nodeArray) { // 计算目标天体
                ...
                if (SituationAssessor.predictedOppStrength(node, team, false) == 0)
                    continue; // 条件2：有敌方
                if (SituationAssessor.predictedGroupStrength(node, team) > SituationAssessor.predictedOppStrength(node, team) * 1.5)
            ...
            if (targets.length > 0) { // 目标天体存在时
                ...
                for each (node in nodeArray) { // 计算出兵天体
                    ...
                    if (SituationAssessor.teamStrength(node, team, false) == 0)
                        continue; // 基本条件：有我方兵力
                    node.aiStrength = -(SituationAssessor.groupStrength(node, team) - SituationAssessor.predictedOppStrength(node, team)); // 将该天体己方强度记为己方强度与预测敌方强度之差的相反数
                    if (EntityContainer.inAttackNodeCheck(node, team, NodeType.PULSECANNON, true)) {
                        if (node.aiStrength > 0)
                            node.aiStrength /= 10;
                        else
                ...
                while (targets.length > 0 && senders.length > 0) {
                    ...
                    if (targetNode && senderNode) {
                        ...
                        if (SituationAssessor.groupStrength(senderNode, team) + SituationAssessor.predictedGroupStrength(targetNode, team) < SituationAssessor.predictedOppStrength(targetNode, team)) {
                        ...
                        ships = (SituationAssessor.predictedOppStrength(targetNode, team, false) * 1.5 - SituationAssessor.predictedGroupStrength(targetNode, team, false)) / shipStrength;
                        if (SituationAssessor.predictedOppStrength(senderNode, team) > SituationAssessor.predictedGroupStrength(senderNode, team))
                            ships = SituationAssessor.teamStrength(senderNode, team, false); // 预测敌方强度大于己方时，派出全部飞船（撤退
            ...
            for each (node in nodeArray) { // 计算目标天体
                ...
                if (SituationAssessor.predictedOppStrength(node, team, false) == 0 && SituationAssessor.predictedGroupStrength(node, team, false) > captureCost * node.nodeData.size * 1.5)
            ...
            if (targets.length > 0) {
                ...
                while (targets.length > 0 && senders.length > 0) {
                    ...
                    if (targetNode && senderNode) {
                        ...
                        ships = (SituationAssessor.predictedOppStrength(targetNode, team) * 2 - SituationAssessor.predictedGroupStrength(targetNode, team)) / shipStrength;
